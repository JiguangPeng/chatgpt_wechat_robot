...
func (g *GroupMessageHandler) ReplyText() error {
	...
	conversation_id = g.service.GetUserSessionContext()

	// 3.请求GPT获取回复
	reply, conversation_id, err = gpt.Completions(g.getRequestText(),conversation_id)
	if err != nil {
		text := err.Error()
	...
	// 4.设置上下文，并响应信息给用户
	g.service.SetUserSessionContext(conversation_id)
